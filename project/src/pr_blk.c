...
static int bd_rw_page(struct block_device *bdev, sector_t sector,
                      struct page *page,
                      enum req_op op); // todo убрать в заголовочный файл
...
static void copy_from_bd(void *dst, struct block_dev *bd, sector_t sector,
                         size_t n);
static int copy_to_bd_setup(struct block_dev *bd, sector_t sector, size_t n,
                            gfp_t gfp);
...
static void copy_from_bd(void *dst, struct block_dev *brd, sector_t sector,
                         size_t n);
static void copy_to_bd(struct block_dev *bd, const void *src, sector_t sector,
                       size_t n);
static int bd_ioctl(struct block_device *bdev, fmode_t mode, unsigned int cmd,
                    unsigned long arg);
static int brd_read_pages_to_buffer(struct block_dev *bd, sector_t sector,
...
static int bd_proc_show(struct seq_file *m, void *v) {
  ...
  seq_printf(m, "%-6s %-10s %-10s %-10s %-12s %-12s %-10s %-10s %-10s %-12s\n",
  ...
  list_for_each_entry(bd, &bd_devices, bd_list) {
    ...
    seq_printf(m,
               "ram%-4d %-10lu %-10llu %-10llu %-12llu %-12llu %-10llu %-10llu "
               "%-10llu %-12llu\n",
               bd->bd_number, size_kb, reads, writes, read_sec, write_sec,
               multi_seg, small_req, errors, avg_size);
  }
...
/* Проверка версии ядра */
    ...
    LINUX_VERSION_CODE > KERNEL_VERSION(6, 1, 255)
...
static int bd_ioctl(struct block_device *bdev, fmode_t mode, unsigned int cmd,
  ...
  case BRD_DUMP: {
    ...
    printk(KERN_INFO "BRD_DUMP: sector=%llu, count=%u, buf_ptr=0x%llx\n",
           (unsigned long long)args.sector, args.count,
...
static int brd_read_pages_to_buffer(struct block_dev *bd, sector_t sector,
  ...
  while (copied < len) {
    ...
    unsigned int to_copy =
        min(len - copied, (unsigned int)(PAGE_SIZE - pg_off));
    struct page *page;
    void *src;
...
в bd_do_bvec() — при чтении или записи данных, чтобы получить страницу, в
которую нужно копировать данные; в copy_to_brd() / copy_from_brd() — при
фактическом копировании данных.*/
static struct page *bd_lookup_page(struct block_dev *bd, sector_t sector) {
...
gfp — флаги аллокации (GFP_NOIO, GFP_NOWAIT, и т.п.), определяют, можно ли спать
...
static int bd_insert_page(struct block_dev *bd, sector_t sector, gfp_t gfp) {
 ...
 alloc_page() — выделяет физическую страницу памяти (struct page).
  Флаги:
  __GFP_ZERO — заполняет страницу нулями (чтобы содержимое RAM-диска было
 изначально чистым).
  __GFP_HIGHMEM — разрешает аллокацию страниц в зоне HIGHMEM (если архитектура
 поддерживает). Если памяти нет → -ENOMEM.
  */
  page = alloc_page(gfp | __GFP_ZERO | __GFP_HIGHMEM);
  if (!page)
    return -ENOMEM;
  /*
  radix_tree_maybe_preload() заранее резервирует память для внутренних узлов
  дерева (чтобы не делать аллокацию под спинлоком).
  */
  ...
  Сохраняем page->index = idx — чтобы знать, какому смещению соответствует
  страница.*/
  ...
  Проверяем, почему:
  Если страница с таким индексом всё-таки уже появилась — возможно, конкурентная
  вставка → просто lookup. Если lookup не возвращает страницу — ошибка
  аллокации. Если index не совпадает — повреждение данных (-EIO).
  */
...
/*
copy_from_bd() выполняет чтение из виртуального RAM-диска в буфер,
предоставленный подсистемой ввода-вывода (bio). dst — адрес назначения (куда
копировать данные, например bio-буфер). brd — RAM-диск (struct brd_device).
...
static void copy_from_bd(void *dst, struct block_dev *brd, sector_t sector,
  ...
  */
  ...
  Если нет — NULL.
  То есть, если мы читаем секторы, которые ещё не записывались — там нет
  страниц, и мы должны вернуть нули.*/
...
/*
bd — указатель на RAM-диск (struct block_dev), содержащий radix-дерево с
данными. sector — начальный сектор операции (позиция в логических секторах). n —
количество байт, которые будут записаны. gfp — флаги выделения памяти (GFP_NOIO,
GFP_NOWAIT, и т.п.) для безопасной аллокации в контексте блочного драйвера.
*/
static int copy_to_bd_setup(struct block_dev *bd, sector_t sector, size_t n,
                            gfp_t gfp) {
  ...
  (sector & (PAGE_SECTORS - 1)) даёт номер сектора внутри текущей страницы
  (0–7).
  << SECTOR_SHIFT (обычно << 9) преобразует этот индекс в байтовое смещение
  внутри страницы.
  */
  unsigned int offset = (sector & (PAGE_SECTORS - 1)) << SECTOR_SHIFT;
  size_t copy;
  ...
  /* PAGE_SIZE - offset — сколько байт помещается в текущую страницу, начиная с
     offset. min_t() гарантирует, что мы не выйдем за пределы данных (n). */
  copy = min_t(size_t, n, PAGE_SIZE - offset);
...
/* функция выполняет одну операцию ввода/вывода для одного сегмента bio (bvec).
    bd — указатель на структуру RAM-диска (struct brd_device), содержащую
   radix-дерево страниц и метаданные. page — страница (структура struct page) из
   BIO (то есть, куда писать или откуда читать). len — длина сегмента в байтах.
...
static int bd_do_bvec(struct block_dev *bd, struct page *page, unsigned int len,
  ...
  /* Макрос op_is_write() проверяет тип операции (REQ_OP_WRITE, REQ_OP_FLUSH, и
   * др.). */
  if (op_is_write(opf)) {

    /*
    Если BIO помечено REQ_NOWAIT, то нельзя блокироваться → используем
    GFP_NOWAIT. Иначе — GFP_NOIO, чтобы запретить I/O при аллокации (во
    избежание рекурсии в блоковый слой)
    */
 ...
 *   которые не хотят ждать.
 ...
 Получить контекст устройства (bd) и стартовый сектор из BIO.
    ...
    при записи — подготовит backing-страницы (alloc_page + radix tree insert),
 затем скопирует данные; при чтении — скопирует данные из backing-страниц в page
 (или заполнит нулями, если страница отсутствует). если brd_do_bvec вернул
 ошибку: если это -ENOMEM и BIO был REQ_NOWAIT — ответить bio_wouldblock_error;
...
static void bd_submit_bio(struct bio *bio) {
  /*
  Извлекаем bd_device через связанное block_device (bio->bi_bdev) → gendisk →
  private_data. То есть каждый gendisk хранит указатель на структуру bd, и здесь
  мы получаем контекст устройства, на котором выполняется этот BIO.
  */
  struct block_dev *bd = bio->bi_bdev->bd_disk->private_data;

  /*
  Читаем стартовый сектор операции из BIO iterator. Это сектор на устройстве, с
  которого начинается I/O. Дальше при обходе сегментов мы будем увеличивать
  sector на число секторов в каждом сегменте.
  */
  ...
  bio_for_each_segment(bvec, bio, iter) {
    ...
    WARN_ON_ONCE((bvec.bv_offset & (SECTOR_SIZE - 1)) ||
                 (len & (SECTOR_SIZE - 1))); // retuen EIO better
    /*
    ...
    brd_do_bvec сделает необходимый kmap_atomic, подготовку backing-страниц (для
    записи), копирование и т.п.
    */
    err =
        bd_do_bvec(bd, bvec.bv_page, len, bvec.bv_offset, bio->bi_opf, sector);
    if (err) {
...
static struct block_dev *bd_alloc_dev(int n) {
  ...
  list_for_each_entry(bd, &bd_devices, bd_list) {
    if (bd->bd_number == n) {
      ...
      /*  Возвращаем ERR_PTR(-EEXIST). Это идиома ядра: вместо NULL возвращается
      указатель, в котором закодирован отрицательный код ошибки. -EEXIST
      означает «устройство уже существует». Использование ERR_PTR позволяет
      вызывающей функции легко отличить ошибку от валидного указателя (через
      IS_ERR() и получить код через PTR_ERR()). */
  ...
  Вызывающая сторона должна проверить, что возвращён не ERR_PTR (через
  IS_ERR()), и затем продолжить полную инициализацию устройства.*/
  return bd;
}
...
static int bd_alloc(int n) {
  ...
  if (!IS_ERR_OR_NULL(bd_debugfs_dir)) {
    debugfs_create_u64(buffer, 0444, bd_debugfs_dir, &bd->bd_nr_pages);
  }
...
static void bd_free_pages(struct block_dev *bd) {
  ...
  do {
    ...
    nr_pages =
        radix_tree_gang_lookup(&bd->bd_pages, (void **)pages, pos, FREE_BATCH);
