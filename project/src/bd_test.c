...
int dump_sectors_to_file(int fd, const char *outfile, __u64 start_sector,
                         __u32 count) {
  ...
  struct brd_dump_args args = {
      .sector = start_sector, .count = count, .buf_ptr = (uintptr_t)buffer};
...
int main(int argc, char *argv[]) {
  if (argc < 2) {
    fprintf(stderr,
            "Usage: %s /dev/ramN [dump <outfile> <start_sector> <count>]\n",
            argv[0]);
