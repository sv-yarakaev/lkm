...
static int reader_thread(void *arg) {
  ...
  while (!kthread_should_stop() && attempts < MAX_ATTEMPTS) {
    ...
    /* Входим в readers-секцию (обычная readers-writer логика) */
    mutex_lock(&rmutex);
    readers_count++;
    if (readers_count == 1) {
      mutex_lock(&rw_mutex);
    ...
    /* Находим запись под защитой rw_mutex */
    record_db_t *rec = record_db_find_by_id(idx);

    /* Если запись найдена — скопируем нужные поля пока мы защищены rw_mutex.
     * Копирование строки предотвращает use-after-free после разблокировки. */
    char *name_copy = NULL;
    long id_copy = 0;
    if (rec) {
      /* kstrdup может спать — это допустимо, mutex (sleeping mutex) позволяет
       * это. Альтернатива — использовать GFP_ATOMIC и лимитированное локальное
       * буферирование. */
      name_copy = kstrdup(rec->name ? rec->name : "", GFP_KERNEL);
      id_copy = rec->id;
    }

    /* Покидаем readers-секцию */
    mutex_lock(&rmutex);
    readers_count--;
    if (readers_count == 0) {
    ...
    /* Теперь безопасно использовать копию (или отметить, что не найдено) */
    if (name_copy == NULL) {
